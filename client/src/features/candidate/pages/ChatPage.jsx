...
export default function ChatPage() {
  ...
  // Load existing applications from API on mount
  useEffect(() => {
    const candidateId = profile?.id
    if (!candidateId) return

    applicationsApi.list({ candidateId })
      .then((apps) => {
        if (!Array.isArray(apps) || apps.length === 0) return

        apps.forEach((app) => {
          const contactId = `employer_${app.employerId}`
          const time = new Date(app.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })

          setContacts((prev) => {
            if (prev.some((c) => c.id === contactId)) return prev
            return [...prev, { id: contactId, name: app.employerName || 'Employer', avatar: '🏢', online: true }]
          })
          ...
          setMessages((prev) => {
            ...
            return {
              ...
              [contactId]: [
                {
                  id: new Date(app.createdAt).getTime(),
                  from: 'me',
                  text: `📋 ${t('chat.appliedFor', { position: app.jobTitle, defaultValue: 'Applied for "{{position}}"' })}`,
                  time,
                },
                {
                  id: new Date(app.createdAt).getTime() + 1,
                  from: 'me',
                  type: 'profile',
  ...
  // Handle incoming application from JobsPage
  useEffect(() => {
    const state = location.state
    if (!state?.newEmployer) return

    const { newEmployer, profile } = state
    const contactId = `employer_${newEmployer.id}`

    // Add employer contact if not already present
    setContacts((prev) => {
      if (prev.some((c) => c.id === contactId)) return prev
      return [...prev, { id: contactId, name: newEmployer.name, avatar: '🏢', online: true }]
    })

    // Create initial messages with profile card
        ...
        [contactId]: [
          {
            id: Date.now(),
            from: 'me',
            text: `📋 ${t('chat.appliedFor', { position: newEmployer.jobTitle, defaultValue: 'Applied for "{{position}}"' })}`,
            time: now,
          },
          {
            id: Date.now() + 1,
        ...
        <div className="flex h-full">
          ...
          <div className={`w-full md:w-1/4 border-r border-gray-100 flex flex-col ${mobileShowChat ? 'hidden md:flex' : 'flex'}`}>
            <div className="p-3 border-b border-gray-100">
              <h3 className="font-semibold text-sm text-gray-900">{t('chat.contacts')}</h3>
            </div>
          ...
          <div className={`flex-1 flex flex-col ${!mobileShowChat ? 'hidden md:flex' : 'flex'}`}>
            {!currentContact ? (
              <div className="flex-1 flex items-center justify-center text-gray-300 text-sm">
                {contacts.length === 0
                  ? t('chat.noContacts', 'No conversations yet. Apply for a job to start chatting.')
            ...
            <div className="px-4 py-3 border-b border-gray-100 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-[#004AAD]">{t('chat.online')}</p>
              </div>
            </div>
