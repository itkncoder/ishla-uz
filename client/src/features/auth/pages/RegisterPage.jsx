...
export default function RegisterPage() {
  ...
  const handleSendOtp = async (e) => {
    ...
    const cleanEmail = sanitizeString(form.email)
    if (!cleanName || cleanName.length < 2) {
      setError(t('nameTooShort', 'Name must be at least 2 characters'))
      return
    }
  ...
  return (
    <div className="w-full max-w-[460px]">
      {/* Card */}
      <div className="bg-white border border-gray-200 px-8 py-8 rounded-xl">

        {step === 'form' ? (
          <>
            <h1 className="text-xl font-semibold text-gray-900 mb-1">{t('registerTitle')}</h1>
          ...
          <>
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-500">{t('otpSentTo')} <span className="font-medium text-gray-700">{form.email}</span></p>
            </div>
            ...
            <form onSubmit={handleVerify} className="space-y-6">
              ...
              <button
                ...
                className="w-full rounded-lg bg-[#004AAD] px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-[#003d8f] focus:outline-none focus:ring-2 focus:ring-[#004AAD] focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
              >
                {isLoading ? <Spinner label={t('otpVerifying')} /> : t('registerButton')}
              </button>
            </form>
        ...
        <div className="text-center">
          <p className="text-sm text-gray-500 mb-3">{t('hasAccount')}</p>
          <Link
            to="/"
